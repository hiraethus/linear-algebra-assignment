...
my.lm.ridge <- function(resp, cova, lambda){
  ...
  if (lambda < 1) errors <- c(errors, "\nlambda value must be greater than or equal to 1")
...
calc.rss <- function(resp, cova, lambda, K=10) {
  ...
  rss.errors <- lapply(X = folds, FUN=function(test.indices) {
    ...
    coefficients <- my.lm.ridge(training.resp, training.cova, lambda)
    beta0 <- coefficients[1]
    coefficients.without.beta0 <- coefficients[-1]
    test.predicted.resp <- (test.cova %*% coefficients.without.beta0) + beta0
