...
% TODO draw the original vectors
<<fig=TRUE>>=
ggplot(data.frame(x=c(3, 3)), aes(x)) +
  geom_segment(aes(x = 0, y = 0, xend = 2, yend = 2), arrow = arrow(length = unit(0.5, "cm"))) +
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 3), arrow = arrow(length = unit(0.5, "cm"))) +
  labs(title="Vectors") +
  xlab("x") +
  ylab("y")  
@
