...
\begin{enumerate}
\item Download and read the prostate cancer dataset prostate.data into a data matrix. The
...
\item Estimate and define a linear model using the $lm()$ function.

\item Split the prostate dataset into a test and training dataset (see column 10)

\item Predict $lpsa$ for the test examples \\
...
\item Plot your results (true $lpsa$ value $y_{i}$ and predicted $\hat{y}_{i}$).

\item Repeat the analysis using a random forest regression and plot your results (true $lpsa$ value $y_{i}$ and predicted $\hat{y}_{i}$).
